...
public class Task2 {
    ...
    double getFractionalPart(double realNumber) {
        return realNumber - Math.floor(realNumber); //your code here
    }
    ...
    boolean numberInRange(Integer number, Integer leftBound, Integer rightBound, Boolean inclusively) {
        ...
        int q = inclusively.booleanValue() ? 0 : 1;
        return (number.compareTo(leftBound) >= q && rightBound.compareTo(number) >= q);
    }
    ...
    boolean areRealNumbersEqual(double a, double b) {
        return Math.abs(a - b) < 1e-9; //your code here
    }
    ...
    int sumOfDigits(int n) {
        return n % 10 + n / 10 % 10 + n / 100; //your code here
    }
    ...
    int nextEvenNumber(int n) {
        return (n & 1) == 0 ? n + 2 : n + 1; //your code here
    }
